...
async function startServer() {
  ...
  app.post("/api/download", async (req, res) => {
    ...
    try {
      ...
      const isWatermarkFree = options && options.includes("Tanpa Watermark");
  ...
  // API Route for Audio/Video to Text Transcript mapping using Gemini
  app.post("/api/transcript", async (req, res) => {
    ...
    try {
      ...
      // 2. Download audio untuk dikirim ke Gemini agar transcript asli
      ...
      try {
        ...
        // Hapus file sementaranya biar gk menuhin disk
        try { fs.unlinkSync(tempPath); } catch(err) {}

        try { await ai.files.delete({ name: uploadResult.name }); } catch(err) {}

        const finalTranscript = aiResponse.text;

        res.json({ transcript: finalTranscript, audioUrl: audioUrl });

      } catch (gem_err: any) {
        try { fs.unlinkSync(tempPath); } catch(err) {}
        console.error("Gemini Upload/Gen Error:", gem_err.message || gem_err);
